...
from .models import Game, GuessStatus, Round
...
def new_game():
    try:
        ...
        schema = GameSchema()
        game = schema.load(
            {"difficulty": difficulty, "range_min": range_min, "range_max": range_max}
        )
        # pprint(schema.dump(game))
        while not game.is_over:
            current_round = game.get_rounds()[-1]  # last round in list
            response = input(
                f"Guess an integer between {current_round.range_min} and {current_round.range_max} (inclusive): "
            )
            try:
                guess = int(response)
